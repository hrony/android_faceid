...
public class UVCCamera {
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
        ...
        try {
            ...
            fd = mCtrlBlock.getFileDescriptor();
            Log.i("serch111", "open mCtrlBlock.getVenderId() = " + mCtrlBlock.getVenderId() + "==mCtrlBlock.getProductId() =" + mCtrlBlock.getProductId() + "--CtrlBlock.getFileDescriptor()=" + mCtrlBlock.getFileDescriptor());
...
//		int setPreviewflag = nativeSetPreviewSize(mNativePtr, DEFAULT_PREVIEW_WIDTH, DEFAULT_PREVIEW_HEIGHT,
//			DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, DEFAULT_PREVIEW_MODE, DEFAULT_BANDWIDTH);

//		Log.i("tomy","open setPreviewflag = "+setPreviewflag);
    }
...
//**********************************************************************
    ...
    //发送命令到设备
    private static final native int nativetSendCmd(final long id_camera, int id, int control, int cmd, int cmd_param, int size);

    //从设备获取相应的状态码
    private static final native int nativeGetStatusFromDevice(final long id_camera, int id, int control, int cmd, int cmd_param, int size);

    //u盘模式转uvc
    private static final native int nativeEsSwitchToUVCCamera(final long id_camera, int vid, int pid, int fd, int busnum, int devnum, String usbfs_str);
    ...
    public int sendCmd(int id, int control, int cmd, int cmd_param, int size) {
        int ret = -1;
        if (mNativePtr != 0) {
            ret = nativetSendCmd(mNativePtr, id, control, cmd, cmd_param, size);
        }
        return ret;
    }

    public int getStatuesFromeDevice(int id, int control, int cmd, int cmd_param, int size) {
        int ret = -1;
        if (mNativePtr != 0) {
            ret = nativeGetStatusFromDevice(mNativePtr, id, control, cmd, cmd_param, size);
        }
        return ret;
    }

    public boolean switchToUVCCamera(UsbControlBlock ctrlBlock) {
        Log.e("tomyx", "switchToUVCCamera");
        UsbDevice device = null;
        if (ctrlBlock != null && ctrlBlock.getDevice() != null) {
            device = ctrlBlock.getDevice();
            mCtrlBlock = ctrlBlock;
            Log.e("changmode", "转换模式111111 device =" + device.toString() + "--ctrlBlock.getFileDescriptor()=" + ctrlBlock.getFileDescriptor());
            int ret = nativeEsSwitchToUVCCamera(mNativePtr, device.getVendorId(), device.getProductId(), ctrlBlock.getFileDescriptor(), ctrlBlock.getBusNum(), ctrlBlock.getDevNum(), UVCCamera.getUSBFSName(ctrlBlock));
            return ret >= 0;
        }
        return false;
    }
    ...
    public boolean setTimeToUvc(int year, int month, int day, int hour, int minute, int second) {
        ...
        int ret = nativeEsSetTime(mNativePtr, second, minute, hour, day, month, year);
    ...
    public String getFWVersion() {
        ...
        if (mNativePtr != 0) {
            String version = nativeEsGetFWVersion(mNativePtr);
            return version;
        }
        return "incar-camera V1.0";
    }
}
