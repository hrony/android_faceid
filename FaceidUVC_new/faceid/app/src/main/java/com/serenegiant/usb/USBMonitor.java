...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	private final void processConnect(final UsbDevice device) {
		Log.w("serch111","processConnect device ="+device.toString());
		if (destroyed) return;
		...
		mAsyncHandler.post(new Runnable() {
			...
			public void run() {
				...
				if (mOnDeviceConnectListener != null) {
					if(device == null){
						Log.e("serch111","---------------deviece null");
					}

					if(ctrlBlock ==null){
						Log.e("serch111","-----------ctrlBlock null");
					}

					if(mOnDeviceConnectListener ==null){
						Log.e("serch111","-------------mOnDeviceConnectListener null");
					}
...
//			}
		...
		public synchronized void close() {
			...
			Log.e("serch111","mConnection.close()");
			if (mConnection != null) {
